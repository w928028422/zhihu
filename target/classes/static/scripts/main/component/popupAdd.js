    ...
    Base.mix(PopupAdd, Component, {
        _tpl: [
            '<div class="zh-add-question-form">',
                ...
                '</div>',
            '<div class="zg-section-big clearfix">',
            '<div class="add-question-section-title">问题分类（可选）：</div>',
            '<div class="zg-form-text-input add-question-title-form" style="position: relative;">',
            '<input type="text" class="js-category zg-editor-input zu-seamless-input-origin-element" placeholder="问题分类" style="height:22px;min-height:auto;"></textarea>',
            '</div>',
            '</div>',
            '</div>'].join(''),
        listeners: [{
            ...
            handler: function () {
                ...
                that.categoryIpt = oEl.find('.js-category');
                // 还原值
    ...
    function fVal(oData) {
        ...
        if (arguments.length === 0) {
            return {
                ...
                category: $.trim(that.categoryIpt.val())
            };
        } else {
            oData = oData || {};
            that.titleIpt.val($.tirm(oData.title));
            that.contentIpt.val($.trim(oData.content));
            that.categoryIpt.val($.trim(oData.category));
        }
    }
